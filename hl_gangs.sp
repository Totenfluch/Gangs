...
public void OnPluginStart() {
	...
	gcv_sDatabase = AutoExecConfig_CreateConVar("hl_gangs_database_name", "gsxh_multiroot", "Name of the database for the plugin.");
	
	gcv_iMaxGangSize = AutoExecConfig_CreateConVar("hl_gangs_max_size", "4", "Initial size for a gang");
	
	gcv_iCreateGangPrice = AutoExecConfig_CreateConVar("hl_gangs_creation_price", "2000", "Price of gang creation");
	
	gcv_iRenamePrice = AutoExecConfig_CreateConVar("hl_gangs_rename_price", "4000", "Price to rename");
...
int GetClientCredits(int client) {
	return tConomy_getCurrency(client);
}

void SetClientCredits(int client, int iAmmount) {
	tConomy_setCurrency(client, iAmmount, "Set By Gangs Plugin");
}
